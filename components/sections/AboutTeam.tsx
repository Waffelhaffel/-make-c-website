...
const DEFAULT_TEAM_IMAGE = "/Team Bild1.JPG";

export function AboutTeam({ data }: AboutTeamProps) {
  const teamImageSrc = hasImageAsset(data.teamImage)
    ? urlFor(data.teamImage).width(1200).quality(85).auto("format").url()
    : DEFAULT_TEAM_IMAGE;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 md:mb-32">
          <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl leading-tight mb-4 md:mb-6">
            {data.quoteLine1 && <span className="font-bold italic">{data.quoteLine1}</span>}
            <br />
            {data.quoteLine2 && (
              <span className="font-garamond font-semibold italic">{data.quoteLine2}</span>
            )}
          </h2>

          <div className="flex justify-center mt-4">
            <div className="w-[120px] md:w-[180px]">
              <svg viewBox="0 0 2048 1020" className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
                <path
                  fill="white"
        ...
        <div className="relative">
          <h2 className="text-4xl md:text-6xl lg:text-7xl text-center mb-0 relative z-20">
            {data.teamTitlePart1 && <span className="font-bold italic">{data.teamTitlePart1}</span>}
            {data.teamTitlePart2 && (
              <span className="font-garamond font-semibold italic">{data.teamTitlePart2}</span>
            )}
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-start -mt-4 md:-mt-8">
            ...
            <div className="flex flex-col justify-start pt-4 sm:pt-8 md:pt-24">
              {data.kicker && (
                <p className="text-makec-blue text-sm md:text-base italic mb-6">{data.kicker}</p>
              )}
